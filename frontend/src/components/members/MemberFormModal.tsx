...
export function MemberFormModal({ member, onClose, onSaved }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Không thể lưu thông tin thành viên');
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700">
          <h2 className="text-lg font-bold text-white">
            {member ? 'Sửa thông tin thành viên' : 'Thêm thành viên mới'}
          </h2>
          <button
            onClick={onClose}
            ...
            <>
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="thanhvien@smashmate.local"
                    className="w-full bg-slate-900 border border-slate-700 rounded-xl pl-9 pr-3 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500 mt-1">
                  * Nếu bỏ trống email/mật khẩu, thành viên sẽ được tạo dưới dạng Member Offline.
                </p>
              </div>
            </>
          )}
